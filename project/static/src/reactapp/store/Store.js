...
class Store extends singleton {
  constructor() {
    ...
    autorun(() => {
      if (!uiStore.isLoading) {
        ...
        const catalogSlug = path[1] === 'catalog' ? path[2] : path[1].length === 0 ? mainCategorySlug : null
  ...
  get productsByCategory() {
    // FIXME: override save method in django model insted
    if (uiStore.catalogFilter !== null) {
      // get all procucts fron descendants categories
      const catalogFilterObj = this.categories.find(c => uiStore.catalogFilter === c.id)
      const catIds = [
        uiStore.catalogFilter,
        ...(catalogFilterObj ? [...catalogFilterObj.descendants.map(c => c.id)] : [])
      ]
      return observable(this.products.filter(product => product.category.some(id => catIds.includes(id))))
    }
  ...
  get filterProductsByPrice() {
    return uiStore.priceFilter.length !== 0
      ? observable(
          this.productsByCategory.filter(
            p => p.minPrice >= uiStore.priceFilter[0] && p.maxPrice <= uiStore.priceFilter[1]
          )
        )
  ...
  @computed
  get popularProducts() {
    return uiStore.catalogFilter !== null ? observable(this.products.filter(product => !!product.isPopular)) : []
  }
  ...
  get maxProductPrice() {
    ...
    return this.products.sort((a, b) => a.maxPrice - b.maxPrice)[this.products.length - 1].maxPrice
  }

  @computed
  get minProductPrice() {
    if (this.products.length === 0) return null
    return this.products.sort((a, b) => a.minPrice - b.minPrice)[0].minPrice
  }
  ...
  findCartItem(product) {
    ...
    return this.cartitems.find(item => {
      return item.product === product.id && item.cartId === this.getCartId && item.property === propertyId
    })
  }
  ...
  get totalPrice() {
    ...
    return filteredByCartId.reduce((sum, current) => {
